 ...
 *
 * Step 2 (in progress): Core modules are being converted from include.module()
 ...
 *   [x] src/smk/layer-display.ts  — LayerDisplay / LayerDisplayContext *   [x] src/smk/query/query.ts     — Query + all subtypes + QueryParameter
 ...
 *   [x] src/smk/viewer-leaflet/viewer-leaflet.ts + all tool leaflet files
 *   [x] src/smk/viewer-esri3d/viewer-esri3d.ts + all tool esri3d files
 *   [x] all tools, config stubs, query/layer subtypes, API modules
 *
 * Grunt still produces dist/smk.js from the unconverted source files. Once
 * all modules are migrated the Grunt build can be retired.
 */
